...
// سه مرحله: شماره → OTP → اسم (فقط کاربر جدید)
...
export default function GuestHeader() {
  ...
  const handleSendOtp = () => {
    ...
    // Mock: ارسال OTP
    console.log("ارسال OTP به", phone);
    setStep("otp");
    startTimer();
  };
  ...
  const handleVerifyOtp = () => {
    ...
    // Mock: تأیید OTP
    // اگه کاربر جدید بود → step name، وگرنه → login
    const isNewUser = true; // بعداً از API میاد
  ...
  const handleFinish = () => {
    ...
    // Mock: ذخیره نام و ورود
    console.log("ثبت‌نام:", { phone, firstName });
    setOpen(false);
    resetForm();
  };
  ...
  bg-black/10 backdrop-blur-xl border-b border-white/5 md:border-transparent">
            ...
            {step === "otp" && (
              <>
                <div className="space-y-1">
                  ...
                  <p className="text-white/40 text-xs">کد ۵ رقمی ارسال شده به {phone} رو وارد کن</p>
                </div>

                {/* ۵ خونه OTP — چپ به راست چون عدده */}
                <div dir="ltr" className="flex gap-2 justify-center">
                  {otp.map((digit, i) => (
                    <input
                      ...
                      className="
                        w-11 h-12 text-center text-white text-lg font-bold
